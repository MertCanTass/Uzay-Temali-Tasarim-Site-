        ...
        <div class="container">
            ...
            <div class="row mt-3">
                <div class="col-12">
                    <div class="row">
                        <div class="col">
                            <h1>Yeni Yayınlanan Fotoğraflar</h1>
                        </div>
                    </div>
                </div>
            ...
            <?php
                   ...
                   $sorgu="SELECT * FROM cards WHERE cardsId BETWEEN 9 and 10";
                   $sonuc=$db->query($sorgu);
                   $kayitSayisi=$sonuc->num_rows;
                   for($i=0; $i<$kayitSayisi; $i++) {
                       $kayit=$sonuc->fetch_assoc();  
               echo '<div class="col-6 mt-2 col-md-12 col-xl-6 col-sm-12">';
                   echo '<div class="card bg-dark text-white">';
                      echo '<img src="'.$kayit["cardResim"].'"class="card-img" alt="...">';
                       echo '<div class="card-img-overlay">';
                       echo '<h5 class="card-title">'.$kayit["cardBaslik"].'</h5>';
                        echo '<p class="card-text">'.$kayit["cardAcıklama"].'</p>';
                        echo '<p class="card-text">'.$kayit["cardGuncelleme"].'</p>';
                      echo '</div>';
                     echo '</div>';
               echo '</div>';
            }
            ...
            <?php
                   $db=new mysqli("localhost","root","","webproje2");
                   mysqli_set_charset($db,"utf8");
                   $sorgu="SELECT * FROM cards WHERE cardsId BETWEEN 11 and 12";
                   $sonuc=$db->query($sorgu);
                   $kayitSayisi=$sonuc->num_rows;
                   for($i=0; $i<$kayitSayisi; $i++) {
                       $kayit=$sonuc->fetch_assoc();  
            ...
            <div class="row mt-2 parca1">
            <?php
                   $db=new mysqli("localhost","root","","webproje2");
                   mysqli_set_charset($db,"utf8");
                   $sorgu="SELECT * FROM cards WHERE cardsId BETWEEN 13 and 14";
                   $sonuc=$db->query($sorgu);
                   $kayitSayisi=$sonuc->num_rows;
                   for($i=0; $i<$kayitSayisi; $i++) {
                       $kayit=$sonuc->fetch_assoc();  
            ...
            <div class="row mt-2 parca2">
            <?php
                   $db=new mysqli("localhost","root","","webproje2");
                   mysqli_set_charset($db,"utf8");
                   $sorgu="SELECT * FROM cards WHERE cardsId BETWEEN 15 and 16";
                   $sonuc=$db->query($sorgu);
                   $kayitSayisi=$sonuc->num_rows;
                   for($i=0; $i<$kayitSayisi; $i++) {
                       $kayit=$sonuc->fetch_assoc();  
            ...
            <div class="row mt-2 parca3">
            <?php
                   $db=new mysqli("localhost","root","","webproje2");
                   mysqli_set_charset($db,"utf8");
                   $sorgu="SELECT * FROM cards WHERE cardsId BETWEEN 17 and 18";
                   $sonuc=$db->query($sorgu);
                   $kayitSayisi=$sonuc->num_rows;
                   for($i=0; $i<$kayitSayisi; $i++) {
                       $kayit=$sonuc->fetch_assoc();  
          ...
          <div class="row mt-2 parca4">
          <?php
                  $db=new mysqli("localhost","root","","webproje2");
                  mysqli_set_charset($db,"utf8");
  
                  $sorgu="SELECT * FROM cards WHERE cardsId BETWEEN 19 and 21";
